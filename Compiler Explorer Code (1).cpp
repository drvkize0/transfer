...
struct InstructionBase
{
    template<typename F, typename T, typename U>
        requires std::same_as<T, Value> && std::same_as<U, Value>
    static constexpr auto evaluate( F f, T lhs, U rhs )
    {
        if( lhs.isShortLayout() )
        {
            if( lhs.isTypeId() )
            {
                return evaluate( f, lhs.getTypeId(), rhs );
            }
            else if( lhs.isNull() )
            {
                return evaluate( f, lhs.getNull(), rhs );
            }
            else if( lhs.isInt() )
            {
                return evaluate( f, lhs.getInt(), rhs );
            }
            else if( lhs.isUInt() )
            {
                return evaluate( f, lhs.getUInt(), rhs );
            }
            else if( lhs.isFloat() )
            {
                return evaluate( f, lhs.getFloat(), rhs );
            }
            else if( lhs.isDouble() )
            {
                return evaluate( f, lhs.getDouble(), rhs );
            }
            else
            {
        ...
        else if( lhs.isReference() )
        {
            return evaluate( f, lhs.getReference(), rhs );
        }
        else if( lhs.isDouble() )
        {
            return evaluate( f, lhs.getDouble(), rhs );
    ...
    static constexpr auto evaluate( F f, T lhs, U rhs )
    {
        if( rhs.isShortLayout() )
        {
            if( rhs.isTypeId() )
            {
                return f( lhs, rhs.getTypeId() );
            }
            else if( rhs.isNull() )
            {
                return f( lhs, rhs.getNull() );
            }
            else if( rhs.isInt() )
            {
                return f( lhs, rhs.getInt() );
            }
            else if( rhs.isUInt() )
            {
                return f( lhs, rhs.getUInt() );
            }
            else if( rhs.isFloat() )
            {
                return f( lhs, rhs.getFloat() );
            }
            else if( rhs.isDouble() )
            {
                return f( lhs, rhs.getDouble() );
            }
            else
            {
                return f( nullptr, nullptr );
...
template<>
struct Instruction<0> : InstructionBase
{
    template<typename T>
    ...
    static inline constexpr Value evaluate( Value lhs, Value rhs )
    {
        return InstructionBase::evaluate( Evaluator(), lhs, rhs );
    }
};
...
int main( int argc, char** argv )
{
    ...
    int result = Instruction<0>::evaluate( Value( 1 ), Value( argc ) ).getInt();
